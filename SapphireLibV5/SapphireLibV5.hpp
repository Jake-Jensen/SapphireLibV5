...
std::string ChangeLineColor(int ForegroundColor, std::string Message, bool OutputToCout, bool UseRandomColor, bool ExcludeBlack);
...
bool CreateDir(std::string DirPath)
{
	...
	int MakeDirRet = _mkdir(DirPath.c_str());
...
std::string ChangeLineColor(int ForegroundColor, std::string Message, bool OutputToCout = false, bool UseRandomColor = false, bool ExcludeBlack = true) {

	if (UseRandomColor) {
		std::random_device dev;
		std::mt19937 rng(dev());
		std::uniform_int_distribution<std::mt19937::result_type> dist6(30, 37);
		std::string RandomColorString;
		std::string PlaceholderForChar;
		for (std::string::size_type i = 0; i < Message.size(); ++i) {
			int RandomColor = dist6(rng);
			if (ExcludeBlack) {
				if (RandomColor == Sapphire::BLACK) {
					RandomColor += 1;
				}
			}
			RandomColorString.append("\033[0;");
			RandomColorString.append(std::to_string(RandomColor));
			RandomColorString.append("m");
			PlaceholderForChar = { Message[i] };
			RandomColorString.append(PlaceholderForChar);
			RandomColorString.append("\033[0m");
		}
		if (OutputToCout) {
			std::cout << RandomColorString << std::endl;
		}
...
void PortScanner()
{
	...
	if (!IsValidIP(DO.IP)) {
		ChangeLineColor(RED, "Invalid IP.", true);
		return;
	}

	if (DO.LowerPort < 0 || DO.LowerPort > 65535) {
		ChangeLineColor(RED, "Invalid low port.", true);
		return;
	}
...
#pragma warning(suppress: 6287)
	if (DO.LowerPort > DO.UpperPort || DO.UpperPort < DO.LowerPort) {
		ChangeLineColor(RED, "Invalid port range. Port range was inverted.", true);
		return;
	}
	...
	if (DO.IsSinglePort) {
		...
		if (IsOpen == "No error" || IsOpen == "Unsupported protocol") {
			...
			ChangeLineColor(GREEN, MessageBuilder, true);
		}
		else {
			std::string MessageBuilder = "Port ";
			MessageBuilder.append(std::to_string(DO.LowerPort));
			...
			MessageBuilder.append(DO.IP);
			MessageBuilder.append(".");
			ChangeLineColor(RED, MessageBuilder, true);
		}
	}
	else {
		for (int i = DO.LowerPort; i < DO.UpperPort + 1; i++) {
			DO.Port = i;
			if (DO.ShowOutput) {
				ChangeLineColor(GREEN, "Scanning port: " + std::to_string(i), true);
			}
			IsOpen = NetworkTool(DO);
			if (IsOpen == "No error" || IsOpen == "Unsupported protocol") {
				std::string MessageBuilder = "Port ";
	...
	ChangeLineColor(WHITE, MessageBuilderX, true);
...
// Clears the terminal, taken from stack overflow.
void ClearConsoleScreen(HANDLE hConsole)
{
